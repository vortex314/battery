...
Usb::Usb(Thread &thr) : Actor(thr), rxdLine(5), txdLine(10), txd(128) //, _poller(thread, 1, true, "poller")
{
    usb1 = this;
...
void Usb::cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
    ...
    usb1->rxdLine.onIsr(Bytes(buf, buf + len));
}
